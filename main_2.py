...
REFERENCE_RANGES = {
    ...
    'BCS': {'min': 4.0, 'max': 6.0, 'units': '1-9 scale'},
    'Weight': {'min': 2.0, 'max': 4.0, 'units': 'kg'}
}
...
def create_timeline_plot(df, animal_id, events_df=None):
    """Create a timeline of medical events for a specific animal"""
    # Get data for the specific animal
    animal_data = df[df['id'] == animal_id].copy()
    
    if len(animal_data) == 0:
        st.warning(f"No data available for {animal_id}.")
        return None
    
    # Get unique dates and significant events
    significant_events = []
    
    # Group tests by date
    date_groups = animal_data.groupby('date')
    
    # For each date, check for abnormal results
    for date, group in date_groups:
        ...
        for _, row in group.iterrows():
            parameter = row['test']
            if parameter in REFERENCE_RANGES:
                if is_outside_range(row['result_numeric'], parameter, row['age_years']):
                    deviation = get_deviation(row['result_numeric'], parameter, row['age_years'])
                    abnormal_results.append(f"{parameter}: {deviation}")
        
        if abnormal_results:
            # If there are any abnormal results, add them to the list of significant events
            significant_events.append({
                'date': date,
                'description': "Abnormal test results: " + ", ".join(abnormal_results[:3]) + 
                            (f" and {len(abnormal_results) - 3} more" if len(abnormal_results) > 3 else ""),
                'type': 'test'
            })
    
    # Add medical events from the events dataframe if provided
    if events_df is not None and not events_df.empty:
        animal_events = events_df[events_df['id'] == animal_id]
        
        for _, row in animal_events.iterrows():
            significant_events.append({
                'date': row['date'],
    ...
    timeline_df = pd.DataFrame(significant_events)
    
    if timeline_df.empty:
        st.info(f"No significant events found for {animal_id}.")
        return None
    ...
    fig = px.timeline(
        ...
        y='description',
        color='type',
        title=f"Medical Timeline for {animal_id}"
    )
...
def merge_datasets(dataframes):
    """Merge multiple dataframes with the same structure"""
    if not dataframes:
        return None
...
def standardize_test_names(df):
    """Standardize test names across different datasets"""
    # Define mappings for commonly misnamed tests
    test_mappings = {
        ...
        'Alkaline Phosphatase': ['ALP', 'ALKP', 'Alk Phosphatase'],
    ...
    for standard, variants in test_mappings.items():
        for variant in variants:
            reverse_mappings[variant.lower()] = standard
    
    # Standardize test names in the dataframe
    if 'test' in df.columns:
        df['test_standardized'] = df['test'].apply(
            lambda x: reverse_mappings.get(str(x).lower(), x) if isinstance(x, str) else x
        )
    
    return df

def detect_medical_events(df, animal_id=None):
    """Detect significant medical events from test results"""
    if animal_id:
        animal_data = df[df['id'] == animal_id].copy()
    else:
        animal_data = df.copy()
    ...
    for (id_val, date), group in animal_data.groupby(['id', 'date']):
        abnormal_tests = []
        age = group['age_years'].iloc[0] if 'age_years' in group.columns else None
        
        # Check for abnormal values
        for _, row in group.iterrows():
            if row['test'] in REFERENCE_RANGES:
                if is_outside_range(row['result_numeric'], row['test'], age):
                    deviation = get_deviation(row['result_numeric'], row['test'], age)
                    test_val = f"{row['result']} {row['units']}" if 'units' in row and row['units'] else str(row['result'])
                    abnormal_tests.append(f"{row['test']}: {test_val} ({deviation})")
        ...
        for _, row in group.iterrows():
            if row['test'] == 'Glucose' and is_outside_range(row['result_numeric'], row['test'], age):
                if get_deviation(row['result_numeric'], row['test'], age) == 'High':
                    glucose_high = True
            
            if row['test'] == 'WBC' and is_outside_range(row['result_numeric'], row['test'], age):
                if get_deviation(row['result_numeric'], row['test'], age) == 'High':
                    wbc_high = True
            
            if row['test'] == 'Creatinine' and is_outside_range(row['result_numeric'], row['test'], age):
                if get_deviation(row['result_numeric'], row['test'], age) == 'High':
                    creatinine_high = True
        ...
        if glucose_high:
            events.append({
                'id': id_val,
                'date': date,
                'event_type': 'Condition Alert',
                'event_description': 'Possible hyperglycemia detected'
            })
        ...
        if wbc_high:
            events.append({
                ...
                'date': date,
                'event_type': 'Condition Alert',
                'event_description': 'Possible infection or inflammation'
            })
        ...
        if creatinine_high:
            events.append({
                ...
                'date': date,
                'event_type': 'Condition Alert',
                'event_description': 'Possible kidney function concern'
            })
        
        # Add event for multiple abnormal tests
        if len(abnormal_tests) >= 3:
            events.append({
                'id': id_val,
                'date': date,
                'event_type': 'Lab Alert',
                'event_description': f'Multiple abnormal values detected ({len(abnormal_tests)} tests)'
            })
...
def create_dashboard():
    ...
    # File uploader for primary dataset
    st.sidebar.subheader("Upload Data")
    uploaded_file1 = st.sidebar.file_uploader(
        "Upload primary CSV file with monkey health data", 
        type="csv", 
        key="file1"
    )
    
    # File uploader for secondary dataset
    uploaded_file2 = st.sidebar.file_uploader(
        "Upload secondary CSV file (optional)", 
        type="csv",
        key="file2"
    )
    
    # File uploader for medical events
    uploaded_events = st.sidebar.file_uploader(
        "Upload medical events CSV (optional)",
        type="csv",
        key="events"
    )
    ...
    with st.sidebar.expander("Expected CSV Format"):
        ...
        For the events CSV (optional):
        - `date`: Date of the event (YYYY-MM-DD)
        - `id`: Unique identifier for the animal
        - `event_type`: Type of event (e.g., 'Surgery', 'Diagnosis')
    ...
    if uploaded_file1 or uploaded_file2:
        ...
        if uploaded_file1:
            try:
                ...
                df1 = standardize_test_names(df1)
                dfs.append(df1)
                st.sidebar.success(f"Successfully loaded primary dataset")
            except Exception as e:
                st.sidebar.error(f"Error loading primary dataset: {e}")
        
        if uploaded_file2:
            try:
                df2 = pd.read_csv(uploaded_file2)
                df2 = standardize_test_names(df2)
                dfs.append(df2)
                st.sidebar.success(f"Successfully loaded secondary dataset")
            except Exception as e:
                st.sidebar.error(f"Error loading secondary dataset: {e}")
    ...
    if uploaded_events:
        try:
            ...
            st.session_state.events_data = events_df
            st.sidebar.success(f"Successfully loaded events data with {len(events_df)} records.")
        except Exception as e:
            st.sidebar.error(f"Error loading events data: {e}")
    ...
    def calculate_anomaly_severity(value, parameter, age=None):
        """Calculate severity score for an anomalous value"""
        range_info = get_reference_range(parameter, age)
        if range_info is None or pd.isna(value):
            return 0
        ...
        if value < range_info['min']:
            ...
            deviation = (range_info['min'] - value) / range_info['min']
        elif value > range_info['max']:
            # Calculate percentage above maximum
            deviation = (value - range_info['max']) / range_info['max']
        else:
            return 0  # Within normal range
        
        # Convert to severity score
        if deviation <= 0.1:
            return 1  # Slight deviation
        elif deviation <= 0.25:
    ...
    if st.session_state.data is not None:
        ...
        # Option to automatically detect events from lab results
        detect_events = st.sidebar.checkbox("Auto-detect medical events from lab results", value=True)
        
        # If auto-detection is enabled, generate events
        if detect_events:
            auto_events = detect_medical_events(data)
            
            # Merge with uploaded events if any
            if st.session_state.events_data is not None:
                all_events = pd.concat([auto_events, st.session_state.events_data], ignore_index=True)
            else:
                all_events = auto_events
        ...
        with tab3:
            ...
            if len(compare_animals) > 0:
                ...
                # Enhanced comparison - adding a time-normalized option
                comparison_type = st.radio(
                    "Comparison Type",
                    ["Over Time", "Most Recent Values", "Statistical Summary"]
                )
                
                if comparison_type == "Over Time":
                    ...
                    if len(compare_data) > 0:
                        fig = px.line(
                            ...
                            color='id',
                            markers=True,
                            title=f"{compare_test} Comparison Over Time"
                        )
                ...
                elif comparison_type == "Most Recent Values":
                    # Get the most recent values for the selected test for each animal
                    most_recent_values = []
                    
                    for animal in compare_animals:
                        animal_data = data[(data['id'] == animal) & (data['test'] == compare_test)]
                        if not animal_data.empty:
                            most_recent = animal_data.sort_values('date').iloc[-1]
                            most_recent_values.append({
                                'Animal': animal,
                                'Value': most_recent['result_numeric'],
                                'Date': most_recent['date'],
                                'Status': get_deviation(most_recent['result_numeric'], compare_test, 
                                                    most_recent['age_years'] if 'age_years' in most_recent else None)
                            })
                    
                    if most_recent_values:
                        compare_df = pd.DataFrame(most_recent_values)
                        ...
                        fig = px.bar(
                            ...
                            y='Value',
                            color='Status',
                            title=f"Most Recent {compare_test} Values",
                    ...
                    # Generate statistical summary for each animal
                    summary_data = []
                    ...
                    for animal in compare_animals:
                        ...
                        if not animal_data.empty:
                            summary_data.append({
                                'Animal': animal,
                                'Count': len(animal_data),
                                'Min': animal_data['result_numeric'].min(),
                                'Max': animal_data['result_numeric'].max(),
                                'Mean': animal_data['result_numeric'].mean(),
                                'Median': animal_data['result_numeric'].median(),
                                'StdDev': animal_data['result_numeric'].std(),
                                'Last Value': animal_data.sort_values('date').iloc[-1]['result_numeric'],
                                'Last Date': animal_data.sort_values('date').iloc[-1]['date']
                            })
                    ...
                    if summary_data:
                        ...
                        animal_test_data = data[(data['id'].isin(compare_animals)) & (data['test'] == compare_test)]
                        if not animal_test_data.empty:
                            fig = px.box(
                                animal_test_data,
                                x='id',
                                y='result_numeric',
                                title=f"Statistical Distribution of {compare_test}",
                                points="all"
                            )
                            st.plotly_chart(fig, use_container_width=True)
                    else:
        ...
        with tab4:
            ...
            if data['id'].nunique() >= 2:
                ...
                try:
                    ...
                    if clustering_method == "Hierarchical Clustering":
                        ...
                        # Get only the numeric columns
                        numeric_cols = pivoted_data.select_dtypes(include=[np.number]).columns.tolist()
                        numeric_cols = [col for col in numeric_cols if col not in ['id', 'age_years']]
                        
                        # Handle missing values
                        analysis_df = pivoted_data[numeric_cols].fillna(pivoted_data[numeric_cols].mean())
                        ...
                        # Add cluster information to the original dataframe
                        clustered_data = pivoted_data.copy()
                        ...
                        if len(numeric_cols) >= 2:
                            feature1 = st.selectbox("Select Feature X", numeric_cols, index=0)
                            feature2 = st.selectbox("Select Feature Y", numeric_cols, index=min(1, len(numeric_cols)-1))
                            ...
                            fig = px.scatter(
                                ...
                                color='cluster',
                                hover_data=['id'],
                                title=f"K-Means Clustering by {feature1} and {feature2}"
                            )
                            st.plotly_chart(fig, use_container_width=True)
                    ...
                    # Add cluster profile analysis
                    st.subheader("Cluster Profiles")
                    
                    # Calculate mean values for each parameter by cluster
                    cluster_profiles = clustered_data.groupby('cluster')[numeric_cols].mean().reset_index()
                    
                    # Select a cluster to view
                    selected_cluster = st.selectbox("Select Cluster to View Profile", 
                                                range(n_clusters),
                                                format_func=lambda x: f"Cluster {x+1}")
                    
                    # Get animals in selected cluster
                    animals_in_cluster = clustered_data[clustered_data['cluster'] == selected_cluster]['id'].tolist()
                    st.write(f"Animals in Cluster {selected_cluster+1}: {', '.join(animals_in_cluster)}")
                    
                    # Profile for selected cluster
                    cluster_profile = cluster_profiles[cluster_profiles['cluster'] == selected_cluster]
                    
                    if not cluster_profile.empty:
                        # Calculate z-scores to see how this cluster differs from overall population
                        cluster_z_scores = {}
                        for col in numeric_cols:
                            overall_mean = pivoted_data[col].mean()
                            overall_std = pivoted_data[col].std()
                            
                            if overall_std > 0:
                                cluster_value = cluster_profile[col].iloc[0]
                                z_score = (cluster_value - overall_mean) / overall_std
                                cluster_z_scores[col] = z_score
                        
                        # Sort by absolute z-score to find most distinctive features
                        sorted_features = sorted(cluster_z_scores.items(), key=lambda x: abs(x[1]), reverse=True)
                        
                        # Display top differentiating features
                        top_features = sorted_features[:5]
                        
                        if top_features:
                            st.write(f"Key characteristics of Cluster {selected_cluster+1}:")
                            feature_desc = []
                            
                            for feature, z in top_features:
                                direction = "higher" if z > 0 else "lower"
                                feature_desc.append(f"- {feature}: {abs(z):.2f} standard deviations {direction} than average")
                            
                            for desc in feature_desc:
                                st.write(desc)
            ...
            if anomalies:
                ...
                anomaly_df['Severity'] = anomaly_df.apply(
                    lambda x: calculate_anomaly_severity(x['Value'], x['Parameter'], 
                                                    data.loc[data['id'] == x['Animal ID'], 'age_years'].iloc[0] 
                                                    if 'age_years' in data.columns else None),
                    axis=1
                )
